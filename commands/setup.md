...
1. First, check if the config file exists at `~/.claude/clicky-keys.env` and read its current values if it does.

2. Ask the user to configure their preferences using AskUserQuestion. Ask all questions in a single AskUserQuestion call with multiple questions:
...
3. Write the configuration to `~/.claude/clicky-keys.env` with this format:
...
4. Ensure the `~/.claude` directory exists before writing the file.

5. Offer to test the sound by running a simple bash command - the hooks will trigger automatically:
   - `sleep 2`
   - If you hear typing sounds during the sleep, the setup is working correctly

6. Confirm the setup is complete and remind the user:
